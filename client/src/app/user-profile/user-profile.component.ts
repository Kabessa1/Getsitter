import { Observable, observable, take, switchMap, BehaviorSubject } from 'rxjs';
...
export class UserProfileComponent implements OnInit {
  profile = new BehaviorSubject<UserProfile | undefined | null>(null);
  constructor(
  ...
  getuserprofile(): Observable<UserProfile> {
    return this.http
      .get<UserProfile>(`/api/user-profile/${this.storageService.getUser().id}`)
      .pipe(take(1));
  }
  ...
  updateMyProfile(): void {
    this.http
      .put(`/api/user-profile/${this.storageService.getUser().id}`, {
      ...
      .subscribe(() => {
        this.profile.next(this.userprofileForm.value);
      });
  }
}
